...
namespace ServiceBus.Tests.ScenarioTests
{
    ...
    {
        [Fact]
        public void RulesCreateGetUpdateDeleteDetail()
        {
            using (MockContext context = MockContext.Start(this.GetType().FullName))
            {
                ...
                Assert.True(ServiceBusTestValidationHelper.ValidateTopicParams(createTopicResponse, getTopicResponse));
               ...
               var ruleName = TestUtilities.GenerateName(ServiceBusManagementHelper.RulesPrefix);
                ...
                // Update Rule with correlation Filter
                var updateRulesParameterToCor = new Rule()
                {
                    SqlFilter = new SqlFilter() { SqlExpression = strSqlExp },
                    FilterType = FilterType.CorrelationFilter,
                    CorrelationFilter = new CorrelationFilter() { Properties = new Dictionary<string, string> { { "topichint1", "topicname1" } } }
                };

                // Get the updated rule to check the Updated values. 
                var updateRulesResponseToCor = ServiceBusManagementClient.Rules.CreateOrUpdate(resourceGroup, namespaceName, topicName, subscriptionName, ruleName, updateRulesParameterToCor);
                Assert.NotNull(updateRulesResponseToCor);
                Assert.Equal(updateRulesParameterToCor.FilterType, updateRulesResponseToCor.FilterType);
                Assert.Equal(updateRulesParameterToCor.CorrelationFilter.Properties, updateRulesResponseToCor.CorrelationFilter.Properties);

                // Update Rule with correlation Filter with added Property of Key-Value pair
                var updateRulesParameterToCorNewProp = new Rule()
                {
                    SqlFilter = new SqlFilter() { SqlExpression = "1=1" },
                    FilterType = FilterType.CorrelationFilter,
                    CorrelationFilter = new CorrelationFilter() { Properties = new Dictionary<string, string> { { "topichint1", "topicname1" }, { "topichint2", "topicname2" } } }
                };

                // Get the updated rule to check the Updated values. 
                var updateRulesResponseToCorNewProp = ServiceBusManagementClient.Rules.CreateOrUpdate(resourceGroup, namespaceName, topicName, subscriptionName, ruleName, updateRulesParameterToCorNewProp);
                Assert.NotNull(updateRulesParameterToCorNewProp);
                Assert.Equal(updateRulesParameterToCorNewProp.FilterType, updateRulesResponseToCorNewProp.FilterType);
                Assert.Equal(updateRulesParameterToCorNewProp.CorrelationFilter.Properties, updateRulesResponseToCorNewProp.CorrelationFilter.Properties);

                // Update Rule with sql Filter and Action again
                var updateRulesParameterToSql = new Rule()
                {
                    Action = new SqlRuleAction()
                    {
                        RequiresPreprocessing = true,
                ...
                // Get the updated rule to check the Updated values. 
                var updateRulesResponseToSql = ServiceBusManagementClient.Rules.CreateOrUpdate(resourceGroup, namespaceName, topicName, subscriptionName, ruleName, updateRulesParameterToSql);
                Assert.NotNull(updateRulesParameterToSql);
                Assert.Equal(updateRulesParameterToSql.FilterType, updateRulesParameterToSql.FilterType);
                Assert.Equal(updateRulesParameterToSql.SqlFilter.SqlExpression, updateRulesParameterToSql.SqlFilter.SqlExpression);

                // Update Rule with correlation Filter without any Property
                var updateRulesParameterToCorNoProp = new Rule()
                {
                    SqlFilter = new SqlFilter() { SqlExpression = "1=1" },
                    FilterType = FilterType.CorrelationFilter,
                    CorrelationFilter = new CorrelationFilter()
                ...
                try
                {
                    var updateRulesResponseToCorNoProp = ServiceBusManagementClient.Rules.CreateOrUpdate(resourceGroup, namespaceName, topicName, subscriptionName, ruleName, updateRulesParameterToCorNoProp);
                }
                catch (Exception e)
                {
                ...
                // Get the updated rule to check the Updated values. 
                getRulesResponse = ServiceBusManagementClient.Rules.Get(resourceGroup, namespaceName, topicName, subscriptionName, ruleName);
                Assert.NotNull(getRulesResponse);
                Assert.Equal(getRulesResponse.Name, ruleName);
